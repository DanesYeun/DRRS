    ...
    <table class="table table-hover table-striped table-borderless">
        <thead class="rounded-top">
            <tr>
                ...
                <th scope="col" class="p-3 bg-primary text-white">Role</th>
        ...
        <tbody id="tableBody">
            @foreach($datas as $data)
                <tr>
                    ...
                    <td class="p-3">{{ $data->role_relation->description }}</td>
                    <td class="p-3 "><small class="badge rounded-pill text-bg-success">{{ $data->status ? 'Active' : '' }}</small></td>
                    <td class="p-3 rounded-end">
                        <a class="btn btn-sm btn-warning text-white" href="{{ route('users.details', ['id' => $data->id]) }}">Edit</a>
